  ...
  var DataBasicPlugin = function(context) {
    ...
    context.memo('button.databasicSize100', function() {
      return ui.button({
        ...
        click: context.createInvokeHandler('usuario.resize', '1'),
      }).render();
    });
    context.memo('button.databasicSize50', function() {
      return ui.button({
        ...
        click: context.createInvokeHandler('usuario.resize', '0.5'),
      }).render();
    });
    context.memo('button.databasicSize25', function() {
      return ui.button({
        ...
        click: context.createInvokeHandler('usuario.resize', '0.25'),
      }).render();
    });
    ...
    self.update = function() {
      ...
      if (!context.invoke('usuario.hasFocus')) {
        self.hidePopover();
        return;
      }

      var rng = context.invoke('usuario.createRange');
      var visible = false;
      ...
      if (rng.isOnData()) {
        ...
        if ($data.length) {
          ...
          // save usuario target to let size buttons resize the container
          context.invoke('usuario.saveTarget', $data[0]);
    ...
    self.createNode = function(info) {
      ...
      if ($node) {
        ...
        info.node = $node;
        // insert node into usuario dom
        context.invoke('usuario.insertNode', $node[0]);
      }
    ...
    self.showDialog = function() {
      ...
      context.invoke('usuario.saveRange');
      ...
      self
        ...
        .then(function(dialogInfo) {
          ...
          context.invoke('usuario.restoreRange');
